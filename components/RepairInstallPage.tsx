...
export const RepairInstallPage: React.FC = () => {
  const service = SERVICES.find(s => s.id === 'repair-install');
  if (!service) return null;

  return (
    <div className="bg-white">
      <PageSEO
        title="Lock Repair & Installation | Pop-A-Lock of Alexandria, LA"
        description="Broken lock? We repair and install deadbolts, knob locks, and smart locks for homes and businesses in Alexandria, LA."
        path="/services/repair-install"
        jsonLd={[LOCAL_BUSINESS_SCHEMA, buildServiceSchema(service, '/services/repair-install')]}
      />
      {/* 1. Page Hero Section */}
